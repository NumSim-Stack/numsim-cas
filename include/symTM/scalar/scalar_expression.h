...
public:
  ...
  scalar_expression(scalar_expression const &data):expression(static_cast<expression const&>(data)){}
  scalar_expression(scalar_expression && data):expression(std::move(static_cast<expression&&>(data))){}
  ...
  constexpr inline auto operator-(){
    ...
    return make_expression<scalar_expression<ValueType>, scalar_negative<ValueType>>(*this);
  }
};

template<typename ValueType>
std::ostream& operator<<(std::ostream & os, expression_holder<scalar_expression<ValueType>> const& expr){
...
struct expression_details<scalar_expression<ValueType>>{
  ...
  static inline auto negative(Expr && expr){
    return make_expression<scalar_negative<ValueType>>(std::forward<Expr>(expr));
  }
};
