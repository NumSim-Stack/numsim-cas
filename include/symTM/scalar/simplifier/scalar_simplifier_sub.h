...
public:
  ...
  auto get_default(){
    ...
    if(rhs_constant){
      add.set_coeff(make_expression<scalar_constant<value_type>>(-m_rhs.template get<scalar_constant<value_type>>()()));
    }else{
...
template<typename T>
class negative_sub final : public sub_default<T>{
public:
  using value_type = T;
  using expr_type = expression_holder<scalar_expression<value_type>>;
  ...
  negative_sub(expr_type lhs, expr_type rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar_negative<value_type>>()}
  {}
...
//    return make_expression<scalar_negative<value_type>>(std::move(add_expr));
//  }
...
public:
  ...
  constant_sub(expr_type lhs, expr_type rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar_constant<value_type>>()}
  {}
...
template<typename T>
class n_ary_sub final : public sub_default<T>{
public:
  using value_type = T;
  using expr_type = expression_holder<scalar_expression<value_type>>;
  using base = sub_default<T>;
  ...
  n_ary_sub(expr_type lhs, expr_type rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar_add<value_type>>()}
  {}
...
public:
  ...
  n_ary_mul_sub(expr_type lhs, expr_type rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar_mul<value_type>>()}
  {}
...
template<typename T>
class symbol_sub final : public sub_default<T>{
public:
  using value_type = T;
  using expr_type = expression_holder<scalar_expression<value_type>>;
  using base = sub_default<T>;
  ...
  symbol_sub(expr_type lhs, expr_type rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar<value_type>>()}
  {}
...
struct sub_base
{
  ...
  constexpr inline expr_type operator()(scalar_negative<value_type> const& lhs){
    return make_expression<scalar_negative<value_type>>(lhs.expr() + m_rhs);
  }
...
////  auto operator()([[maybe_unused]] scalar_one<value_type> &lhs, [[maybe_unused]] Scalar &rhs) {
////    return default_sub();
////  }

////  template<typename Scalar>
////    requires (!isScalarConstant<Scalar> || !isScalarZero<Scalar>)
////  auto operator()([[maybe_unused]] Scalar &lhs, [[maybe_unused]] scalar_one<value_type> &rhs) {
////    return default_sub();
////  }
...
//  auto operator()([[maybe_unused]] scalar<value_type> &lhs, [[maybe_unused]] scalar_sub<value_type> &rhs) {
//    auto sub{make_expression<scalar_sub<value_type>>(rhs)};
//    sub.template get<scalar_sub<value_type>>().push_back(m_lhs);
//    return std::move(sub);
//  }
...
//  template<typename Scalar>
//  auto operator()(scalar_negative<value_type> &lhs, scalar_sub<value_type> &rhs) {
...
//////         //  template <typename Scalar>
//////         //  auto operator()(Scalar &lhs, scalar_constant<value_type> &rhs) {
//////         //    auto mul{make_expression<scalar_mul<value_type>>()};
//////         //    mul.template get<scalar_mul<value_type>>().set_coeff(std::forward<ExprRHS>(m_rhs));
//////         //    mul.template get<scalar_mul<value_type>>().push_back(std::forward<ExprLHS>(m_lhs));
//////         //    return mul;
//////         //  }

//////         //  template <typename Scalar>
//////         //  auto operator()(scalar_constant<value_type> &lhs, Scalar &rhs) {
//////         //    auto mul{make_expression<scalar_mul<value_type>>()};
//////         //    mul.template get<scalar_mul<value_type>>().push_back(m_rhs);
//////         //    mul.template get<scalar_mul<value_type>>().set_coeff(m_lhs);
//////         //    return mul;
//////         //    //return std::forward<ExprRHS>(m_rhs) + std::forward<ExprLHS>(m_lhs);
//////         //  }

//////         //  auto operator()(scalar_mul<value_type> &lhs, scalar_one<value_type> & rhs) {
//////         //    return std::forward<ExprLHS>(m_lhs);
//////         //  }

//////         //  auto operator()(scalar_one<value_type> &lhs, scalar_mul<value_type> & rhs) {
//////         //    return std::forward<ExprRHS>(m_rhs);
//////         //  }

//////         //  auto operator()(scalar_constant<value_type> &lhs, scalar_one<value_type> & rhs) {
//////         //    return std::forward<ExprLHS>(m_lhs);
//////         //  }

//////         //  auto operator()(scalar_one<value_type> &lhs, scalar_constant<value_type> & rhs) {
//////         //    return std::forward<ExprRHS>(m_rhs);
//////         //  }

//////         //  auto operator()(scalar_mul<value_type> &lhs, scalar_zero<value_type> & rhs) {
//////         //    return std::forward<ExprRHS>(m_rhs);
//////         //  }

//////         //  auto operator()(scalar_zero<value_type> &lhs, scalar_mul<value_type> & rhs) {
//////         //    return std::forward<ExprLHS>(m_lhs);
//////         //  }

//////         //  auto operator()(scalar<value_type> &lhs, scalar_zero<value_type> & rhs) {
//////         //    return std::forward<ExprRHS>(m_rhs);
//////         //  }

//////         //  auto operator()(scalar_zero<value_type> &lhs, scalar<value_type> & rhs) {
//////         //    return std::forward<ExprLHS>(m_lhs);
//////         //  }

//////         //  auto operator()(scalar_constant<value_type> &lhs, scalar_zero<value_type> & rhs) {
//////         //    return std::forward<ExprRHS>(m_rhs);
//////         //  }

//////         //  auto operator()(scalar_zero<value_type> &lhs, scalar_constant<value_type> & rhs) {
//////         //    return std::forward<ExprLHS>(m_lhs);
//////         //  }
...
//////         //    auto expr{make_expression<scalar_mul<value_type>>(lhs)};
//////         //    expr.template get<scalar_mul<value_type>>().push_back(std::forward<ExprRHS>(m_rhs));
//////         //    return expr;
//////         //  }

//////         //  //  template <typename Scalar>
//////         //  //  expr_type operator()(Scalar &lhs, scalar_mul<value_type> &rhs) {
//////         //  //    return std::forward<ExprRHS>(m_rhs) + std::forward<ExprLHS>(m_lhs);
//////         //  //  }
...
//  //0 - expr --> -expr
//  template<typename Scalar>
//  auto operator()([[maybe_unused]] scalar_zero<value_type> &lhs, [[maybe_unused]] Scalar &rhs) {
//    return make_expression<scalar_negative<value_type>>(m_rhs);
//  }

//  //expr - 0 --> expr
//  template<typename Scalar>
//  auto operator()([[maybe_unused]] Scalar &lhs, [[maybe_unused]] scalar_zero<value_type> &rhs) {
//    return m_lhs;
//  }
...
////  template<typename Scalar> requires(!isScalarConstant<Scalar> && !isScalarZero<Scalar>)
////  auto operator()([[maybe_unused]] scalar_one<value_type> &lhs, [[maybe_unused]] Scalar &rhs) {
////    return default_sub();
////  }
...
////  //expr - 0 --> expr
////  auto operator()([[maybe_unused]] scalar_sub<value_type> &lhs, [[maybe_unused]] scalar_zero<value_type> &rhs) {
////    return m_lhs;
////  }
...
////  //0 - (-expr) --> expr
////  auto operator()([[maybe_unused]] scalar_zero<value_type> &lhs, [[maybe_unused]] scalar_negative<value_type> &rhs) {
////    return rhs.expr();
////  }

////  //-expr - 0 --> -expr
////  auto operator()([[maybe_unused]] scalar_negative<value_type> &lhs, [[maybe_unused]] scalar_zero<value_type> &rhs) {
////    return m_lhs;
////  }

////  //symbol - 0 --> symbol
////  auto operator()([[maybe_unused]] scalar<value_type> &lhs, [[maybe_unused]] scalar_zero<value_type> &rhs) {
////    return m_lhs;
////  }

////  //0 - symbol --> -symbol
////  auto operator()([[maybe_unused]] scalar_zero<value_type> &lhs, [[maybe_unused]] scalar<value_type> &rhs) {
////    return make_expression<scalar_negative<value_type>>(m_rhs);
////  }


////  auto operator()([[maybe_unused]] scalar_constant<value_type> & lhs, [[maybe_unused]] scalar_div<value_type> & rhs) {
////    return default_sub();
////  }

////  auto operator()([[maybe_unused]] scalar_constant<value_type> & lhs, [[maybe_unused]] scalar_add<value_type> & rhs) {
////    return default_sub();
////  }

////  auto operator()([[maybe_unused]] scalar_constant<value_type> & lhs, [[maybe_unused]] scalar_mul<value_type> & rhs) {
////    return default_sub();
////  }

////  auto operator()([[maybe_unused]] scalar_constant<value_type> & lhs, [[maybe_unused]] scalar_negative<value_type> & rhs) {
////    return default_sub();
////  }
...
//    std::cout<<std::boolalpha<<isScalarConstant<std::remove_reference_t<ScalarLHS>><<" "<<isScalarOne<std::remove_reference_t<ScalarRHS>><<std::endl;
//    std::cout<<std::boolalpha<<isScalarConstant<std::remove_reference_t<ScalarRHS>><<" "<<isScalarOne<std::remove_reference_t<ScalarLHS>><<std::endl;
//    std::cout<<std::is_same_v<std::remove_cvref_t<ScalarLHS>, scalar_constant<value_type>><<std::endl;
//    std::cout<<std::is_same_v<std::remove_cvref_t<ScalarRHS>, scalar_constant<value_type>><<std::endl;
//    std::cout<<std::is_same_v<std::remove_cvref_t<ScalarLHS>, scalar_one<value_type>><<std::endl;
//    std::cout<<std::is_same_v<std::remove_cvref_t<ScalarRHS>, scalar_one<value_type>><<std::endl;
//    return default_sub();
//  }
