...
public:
  ...
  add_default(expr_type lhs, expr_type rhs):m_lhs(lhs),m_rhs(rhs){}
...
template<typename T>
class constant_add final : public add_default<T>{
public:
  using value_type = T;
  using expr_type = expression_holder<scalar_expression<value_type>>;
  ...
  constant_add(expr_type lhs, expr_type rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar_constant<value_type>>()}
  {}
  ...
  constexpr inline expr_type operator()([[maybe_unused]]scalar_add<value_type> const& rhs){
...
template<typename T>
class n_ary_add final : public add_default<T>{
public:
  using value_type = T;
  using expr_type = expression_holder<scalar_expression<value_type>>;
  using base = add_default<T>;
  ...
  n_ary_add(expr_type lhs, expr_type rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar_add<value_type>>()}
  {}
  ...
  constexpr inline expr_type operator()([[maybe_unused]]scalar_one<value_type> const& ){
    auto add_expr{make_expression<scalar_add<value_type>>(lhs)};
    auto &add{add_expr.template get<scalar_add<value_type>>()};
...
template<typename T>
class symbol_add final : public add_default<T>{
public:
  using value_type = T;
  using expr_type = expression_holder<scalar_expression<value_type>>;
  using base = add_default<T>;
  ...
  symbol_add(expr_type lhs, expr_type rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar<value_type>>()}
  {}
...
struct add_base
{
  ...
  add_base(expr_type lhs, expr_type rhs):m_lhs(lhs),m_rhs(rhs){}

  constexpr inline expr_type operator()(scalar_constant<value_type> const&){
    return visit(constant_add<value_type>(m_lhs,m_rhs), *m_rhs);
  }

  constexpr inline expr_type operator()(scalar_add<value_type> const&){
    return visit(n_ary_add<value_type>(m_lhs,m_rhs), *m_rhs);
  }

  constexpr inline expr_type operator()(scalar_mul<value_type> const&){
    return visit(n_ary_mul_add<value_type>(m_lhs,m_rhs), *m_rhs);
  }

  constexpr inline expr_type operator()(scalar<value_type> const&){
    return visit(symbol_add<value_type>(m_lhs,m_rhs), *m_rhs);
  }
  ...
  constexpr inline expr_type operator()(Type const&){
    return visit(add_default<value_type>(m_lhs,m_rhs), *m_rhs);
  }
...
//  using RuleMap = std::unordered_map<std::pair<type_id, type_id>, SimplifyRuleFn>;
...
//template<typename ValueType, typename TypeLHS, typename TypeRHS, typename Func>
//static auto register_binary_simplify_scalar(Func function){
//  scalar_simplify_rules<ValueType>::instance().template register_rule<TypeLHS, TypeRHS>(std::forward<Func>(function));
//  return true;
//}

//template<typename ValueType, template<typename > class... Args, typename Func>
//static auto register_binary_simplify_scalar_var(Func function){
//  scalar_simplify_rules<ValueType>::instance().template register_rule<Args<ValueType>...>(std::forward<Func>(function));
//  return true;
//}
...
//auto add_constant_constant(expression_holder<scalar_expression<ValueType>> const&lhs,
//                          expression_holder<scalar_expression<ValueType>> const&rhs){
...
///// constant - constant = constant
//template<typename ValueType>
//auto sub_constant_constant(expression_holder<scalar_expression<ValueType>> const&lhs,
//                           expression_holder<scalar_expression<ValueType>> const&rhs){
//  using type = scalar_constant<ValueType>;
//  const auto value{lhs.template get<type>()() - rhs.template get<type>()()};
//  return make_expression<scalar_constant<ValueType>>(value);
...
///// constant + negative
//template<typename ValueType>
//auto add_constant_negative(expression_holder<scalar_expression<ValueType>> const&lhs,
//                           expression_holder<scalar_expression<ValueType>> const&rhs){
//  using constant = scalar_constant<ValueType>;
//  using negative = scalar_negative<ValueType>;
//  using add = scalar_add<ValueType>;
//  const auto key{scalar_simplify_rules<ValueType>::instance().get_key(constant::get_id(), add::get_id(), negative::get_id(), get_id(rhs))};
//  return scalar_simplify_rules<ValueType>::instance().run(key, lhs, rhs);
//}

///// negative + constant
//template<typename ValueType>
//auto add_negative_constant(expression_holder<scalar_expression<ValueType>> const&lhs,
//                           expression_holder<scalar_expression<ValueType>> const&rhs){
//  using constant = scalar_constant<ValueType>;
//  using negative = scalar_negative<ValueType>;
//  using add = scalar_add<ValueType>;
//  const auto key{scalar_simplify_rules<ValueType>::instance().get_key(negative::get_id(), get_id(lhs), add::get_id(), constant::get_id())};
//  return scalar_simplify_rules<ValueType>::instance().run(key, lhs, rhs);
//}


////template<typename ValueType>
////auto add_scalar_constants(expression_holder<scalar_expression<ValueType>> const&lhs,
////                          expression_holder<scalar_expression<ValueType>> const&rhs){
////  using type = scalar_constant<ValueType>;
////  std::cout<<"const count lhs "<<lhs.data().use_count()<<std::endl;
////  std::cout<<"const count rhs "<<lhs.data().use_count()<<std::endl;
////  const auto value{lhs.template get<type>()() + rhs.template get<type>()()};
////  return make_expression<scalar_constant<ValueType>>(value);
////}

////auto temp = register_binary_simply<double, scalar_constant<double>, scalar_constant<double>>(
////    [](expression_holder<scalar_expression<double>> const& lhs, expression_holder<scalar_expression<double>> const& rhs){
////      using type = scalar_constant<double>;
////      const auto value{lhs.template get<type>()() + rhs.template get<type>()()};
////  return make_expression<scalar_constant<double>>(value);
////    });
...
//    double, add_constant_negative, scalar_constant, scalar_negative, scalar_constant);

////}
////SYMTM_REGISTER_BINARY_SIMPLIFY(
////    float, scalar_constant, scalar_negative, scalar_constant, scalar_rules::add_constant_negative);

////SYMTM_REGISTER_BINARY_SIMPLIFY(
////    int, scalar_constant, scalar_negative, scalar_constant, scalar_rules::add_constant_negative);


template <typename ExprLHS, typename ExprRHS> class scalar_simplifier_add;

template<typename T>
auto binary_simplify_add(expression_holder<scalar_expression<T>> const& lhs, expression_holder<scalar_expression<T>> const& rhs){
  scalar_simplifier_add<expression_holder<scalar_expression<T>> const&, expression_holder<scalar_expression<T>> const&> visitor(lhs, rhs);
...
//template<typename T>
//expression_holder<scalar_expression<T>> simplify_add(expression_holder<scalar_expression<T>> const& lhs, expression_holder<scalar_expression<T>> const& rhs) {
...
//      : m_lhs(std::forward<ExprLHS>(lhs)), m_rhs(rhs) {}
  ...
  auto operator()([[maybe_unused]]scalar_constant<value_type> &lhs,
    ...
    //return std::visit(simplifier::constant_add<value_type>(m_lhs,m_rhs), *m_rhs);
    //auto func{scalar_simplify_rules<value_type>::instance().lookup(lhs.get_id(), rhs.get_id())};
    //return func(m_lhs, m_rhs);
    //const auto value{lhs() + rhs()};
    //return make_expression<scalar_constant<value_type>>(value);
  }
  ...
  auto operator()(scalar_constant<value_type> &lhs,
    ...
    if(is_same<scalar_constant<value_type>>(rhs.expr())){
      const auto value{lhs() - rhs.expr().template get<scalar_constant<value_type>>()()};
      return make_expression<scalar_constant<value_type>>(value);
    }
  ...
  auto operator()(scalar_negative<value_type> &lhs,
    ...
    if(is_same<scalar_constant<value_type>>(lhs.expr())){
      const auto value{rhs() - lhs.expr().template get<scalar_constant<value_type>>()()};
      return make_expression<scalar_constant<value_type>>(value);
    }
  ...
  auto operator()([[maybe_unused]] scalar_add<value_type> &lhs,
    ...
    if constexpr (std::is_lvalue_reference_v<ExprLHS>) {
      ...
      auto add_expr{make_expression<scalar_add<value_type>>(lhs)};
      auto &add{add_expr.template get<scalar_add<value_type>>()};
      auto coeff{add.coeff() + (m_negative ? -1 : 1) * std::forward<ExprRHS>(m_rhs)};
  ...
  auto operator()([[maybe_unused]] Scalar &lhs, [[maybe_unused]] scalar_constant<value_type> &rhs) {
    auto add_new{make_expression<scalar_add<value_type>>()};
    add_new.template get<scalar_add<value_type>>().push_back(
        std::forward<ExprLHS>(m_lhs));
    add_new.template get<scalar_add<value_type>>().set_coeff(
  ...
  template <typename Scalar>
  expr_type operator()([[maybe_unused]] scalar_constant<value_type> &rhs, [[maybe_unused]] Scalar &lhs) {
    return std::forward<ExprRHS>(m_rhs) + std::forward<ExprLHS>(m_lhs);
  }
  ...
  auto operator()(scalar_negative<value_type> &lhs, scalar_negative<value_type> &rhs) {
    const auto &hash_rhs{rhs.expr().get().hash_value()};
  ...
  auto operator()([[maybe_unused]] scalar<value_type> &lhs, [[maybe_unused]] scalar_mul<value_type> &rhs) {
    return std::forward<ExprRHS>(m_rhs) + std::forward<ExprLHS>(m_lhs);
  }
  ...
  auto operator()(scalar_add<value_type> &lhs, scalar<value_type> &rhs) {
    ...
    if (pos != lhs.hash_map().end()) {
      auto expr{std::visit(
          scalar_simplifier_add<expr_type &, expr_type &>(pos->second, m_rhs),*pos->second, *m_rhs)};
      add.hash_map().erase(rhs.hash_value());
      add.push_back(expr);
      return expr_add;
    }
  ...
  auto operator()([[maybe_unused]] scalar_add<value_type> &lhs, scalar_negative<value_type> &rhs) {
  ...
  auto operator()([[maybe_unused]] ScalarExpr &lhs, [[maybe_unused]] scalar_zero<value_type> &rhs) {
    return m_lhs;
  }

  template<typename ScalarExpr>
  auto operator()([[maybe_unused]] scalar_zero<value_type> &lhs, [[maybe_unused]] ScalarExpr &rhs) {
    return m_rhs;
  }

  auto operator()([[maybe_unused]] scalar_zero<value_type> &lhs, [[maybe_unused]] scalar_zero<value_type> &rhs) {
    return m_rhs;
  }

  auto operator()([[maybe_unused]] scalar_constant<value_type> &lhs, [[maybe_unused]] scalar_zero<value_type> &rhs) {
    return m_lhs;
  }

  auto operator()([[maybe_unused]] scalar_zero<value_type> &lhs, [[maybe_unused]] scalar_constant<value_type> &rhs) {
    return m_rhs;
  }

  auto operator()([[maybe_unused]] scalar_add<value_type> &lhs, [[maybe_unused]] scalar_zero<value_type> &rhs) {
    return m_lhs;
  }

  auto operator()([[maybe_unused]] scalar_zero<value_type> &lhs, [[maybe_unused]] scalar_add<value_type> &rhs) {
    return m_rhs;
  }

  template <typename ScalarLHS, typename ScalarRHS>
  auto operator()([[maybe_unused]] ScalarLHS& lhs, [[maybe_unused]] ScalarRHS& rhs) {
...
private:
  ...
  auto combine() {
    ...
    mul_new.template get<scalar_mul<value_type>>().push_back(
        std::forward<ExprLHS>(m_lhs));
    mul_new.template get<scalar_mul<value_type>>().set_coeff(make_expression<scalar_constant<value_type>>(2));
...
//template <typename ExprLHS, typename ExprRHS> class scalar_simplifier_add_main {
//public:
//  using value_type = typename std::remove_reference_t<
//      std::remove_const_t<ExprLHS>>::value_type;
//  using expr_type = expression_holder<scalar_expression<value_type>>;
...
//    //scalar_simplifier_add<ExprLHS, ExprRHS> visitor(std::forward<ExprLHS>(m_lhs), std::forward<ExprRHS>(m_rhs));
//    return std::visit(
//        [&](auto&, auto&){
//          return default_add();
//          //return expr_type();/*default_add();*/
//        }, lhs.data(), rhs.data());
//    //return std::visit(overload{visitor, [&visitor](auto&, auto&){return visitor.default_add();}}, lhs.data(), rhs.data());
//  }
