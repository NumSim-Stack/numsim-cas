...
class tensor_symmetry final
    : public unary_op<tensor_symmetry<ValueType>, tensor_expression<ValueType>>
{
public:
  using base = unary_op<tensor_symmetry<ValueType>, tensor_expression<ValueType>>;

  template <typename Expr>
  explicit tensor_symmetry(Expr && _expr)
      : base(std::forward<Expr>(_expr),call_tensor::dim(this->m_expr), call_tensor::rank(this->m_expr)),
        m_symmetries()
  {}
