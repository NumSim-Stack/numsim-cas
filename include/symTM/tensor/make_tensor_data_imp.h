...
class make_tensor_data_imp final
    : public tensor_data_eval_up_unary<make_tensor_data_imp<ValueType>, ValueType>
{
public:
  ...
  [[nodiscard]] std::unique_ptr<tensor_data_base<ValueType>> evaluate_imp() noexcept {
    return std::make_unique<tensor_data<ValueType, Dim, Rank>>();
  }

  [[nodiscard]] std::unique_ptr<tensor_data_base<ValueType>> missmatch(std::size_t dim,
                                                            std::size_t rank) {
