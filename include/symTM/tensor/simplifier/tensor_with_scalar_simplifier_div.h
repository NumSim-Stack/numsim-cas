...
protected:
  auto get_default(){
    auto div_new{make_expression<tensor_scalar_div<value_type>>(std::forward<ExprLHS>(m_lhs),std::forward<ExprRHS>(m_rhs))};
...
struct tensor_div_simplifier final : public div_default<ExprLHS, ExprRHS>
{
  ...
  tensor_div_simplifier(ExprLHS lhs, ExprRHS rhs):base(std::forward<ExprLHS>(lhs), std::forward<ExprRHS>(rhs)),m_expr(m_lhs.template get<tensor_scalar_div<value_type>>()){}

  // (A/b)/(c/d) --> A*d/(b*c)
  constexpr inline expr_type operator()(tensor_scalar_div<value_type> const& rhs){
    return make_expression<tensor_scalar_div<value_type>>(m_expr.expr_lhs()*rhs.expr_rhs(), m_expr.expr_rhs()*rhs.expr_lhs());
  }

  // a/b/c --> a/(b*c)
  template<typename Expr,
            std::enable_if_t<std::is_base_of_v<scalar_expression<value_type>,Expr>,bool> = true>
  constexpr inline expr_type operator()(Expr const&){
    return make_expression<tensor_scalar_div<value_type>>(m_expr.expr_lhs(), m_expr.expr_rhs()*m_rhs);
  }
...
struct div_base
{
  ...
  template<typename Expr,
            std::enable_if_t<std::is_base_of_v<tensor_expression<value_type>,Expr>,bool> = true>
  constexpr inline expr_type operator()(Expr const&){
    auto& expr_rhs{*m_rhs};
  ...
  constexpr inline expr_type operator()(tensor_scalar_div<value_type> const&){
    auto& expr_rhs{*m_rhs};
    return visit(tensor_div_simplifier<ExprLHS, ExprRHS>(std::forward<ExprLHS>(m_lhs),std::forward<ExprRHS>(m_rhs)), expr_rhs);
  }
