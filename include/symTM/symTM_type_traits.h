...
//class variant_wrapper : public expression_crtp<variant_wrapper<VariantType, ExprType>, ExprType>
//{
//public:
//  using base_expr =
...
template<typename ExprType>
using umap = std::map<std::size_t, ExprType>;

//template<typename _Visitor, typename... _Variants>
//constexpr std::__detail::__variant::__visit_result_t<_Visitor, _Variants...>
//visit(_Visitor&& __visitor, _Variants&&... __variants){
//  return std::visit(std::forward<_Visitor>(__visitor), std::forward<_Variants>(__variants)...);
//}
...
using scalar_node =
    std::variant<
        scalar<ValueType>,
        scalar_zero<ValueType>,
        scalar_one<ValueType>,
        ...
        scalar_cos<ValueType>,
        scalar_tan<ValueType>,
        scalar_asin<ValueType>,
        scalar_acos<ValueType>,
        scalar_atan<ValueType>,
        //scalar_special_math_functions := {pow, sqrt, log, ln, e^expr, sign, abs}
        scalar_pow<ValueType>,
        scalar_sqrt<ValueType>,
        scalar_log<ValueType>,
        //scalar_ln<ValueType>,
        scalar_exp<ValueType>,
        scalar_sign<ValueType>,
        scalar_abs<ValueType>>;
...
using tensor_to_scalar_node = std::variant<
    tensor_trace<ValueType>,
    tensor_det<ValueType>,
    tensor_dot<ValueType>,
    tensor_norm<ValueType>,
    tensor_to_scalar_negative<ValueType>,
...
//template <typename ValueType>
//struct expression_holder_data_type<scalar_expression<ValueType>, scalar_sin<ValueType>> {
...
template <typename T> struct get_type<std::shared_ptr<T>> { using type = T; };
template <typename T> struct get_type<expression_holder<T>> { using type = T; };
//template <typename T> struct get_type<variant_wrapper<scalar_special_node<T>, scalar_expression<T>>> { using type = scalar_expression<T>; };
//template <typename T> struct get_type<scalar_special_node<T>> { using type = scalar_expression<T>; };
...
///
template <typename T>
concept has_variant_type = requires {
  typename T::variant_type;
};
template <typename T>
concept is_variant_type_index = requires {
  typename T::type;
};
...
struct variant_index_inner<T, std::variant<T, Types...>> : std::integral_constant<std::size_t, 0> {};

template <typename T, typename U, typename... Types>
struct variant_index_inner<T, std::variant<U, Types...>>
    : std::integral_constant<std::size_t, 1 + variant_index_inner<T, std::variant<Types...>>::value> {};
...
private:
  ...
  struct helper<Index, First, Rest...> {
    static constexpr bool found = variant_index_inner<T, typename First::variant_type>::value < std::variant_size<typename First::variant_type>::value;
    static constexpr std::size_t value = found ? Index : helper<Index + 1, Rest...>::value;
  };

  template <std::size_t Index>
  struct helper<Index> { static constexpr std::size_t value = -1; };

public:
  static constexpr std::size_t value = helper<0, SubVariants...>::value;
};

//template <std::size_t Index, typename T> struct result : public std::true_type {
...
//    : public std::conditional<variant_index_inner<0, T, typename First::variant_type>::value,
//                              result<Index, First>,
//                              variant_index_outer<Index+1, T, std::variant<Args...>>>::type
//{};
...
//struct variant_index_inner_loop : public variant_index<0, T, typename Wrapper::variant_type>, public std::false_type
//{};
...
template <std::size_t Index, typename T, typename... Args>
struct variant_index<Index, T, std::variant<T, Args...>> : public std::true_type{
  [[maybe_unused]] static constexpr std::size_t index = Index;
};
...
//      std::in_place_index<variant_index<variant, T>()>, std::forward<Args>(args)...));
//}
...
//  return expression_holder<expr_type>(std::make_shared<variant>(std::in_place_index<variant_index<variant, trigonometric_functions<value_type>>()>,
...
struct base_expression<EXPR, EXPR>{
  using type = EXPR;
};

template<typename ValueType>
struct base_expression<scalar_expression<ValueType>, tensor_expression<ValueType>>{
...
template<typename ValueType>
struct base_expression<tensor_to_scalar_expression<ValueType>, scalar_expression<ValueType>>{
  using type = tensor_to_scalar_expression<ValueType>;
};

template<typename ValueType>
struct base_expression<tensor_expression<ValueType>, tensor_to_scalar_expression<ValueType>>{
  using type = tensor_expression<ValueType>;
};
...
template<class T>
concept isScalarOne = std::is_same_v<std::remove_cvref_t<T>, scalar_one<typename T::value_type>>;
template<class T>
concept isScalarZero = std::is_same_v<std::remove_cvref_t<T>, scalar_zero<typename T::value_type>>;
template<class T>
