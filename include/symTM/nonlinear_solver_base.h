...
public:
  ...
  constexpr inline auto operator()(expression_holder<scalar_expression<ValueType>> & expr){
...
public:
  ...
  template<typename Expr, typename = typename std::enable_if< (... || std::is_base_of_v<Args, symTM::get_type_t<Expr>>) >::type >
...
using nonlinear_scalar_solver = nonlinear_solver_base<double,
                                                      symTM::visitor_diff,
                                                      symTM::visitor_eval,
