...
//   using expr_type =
//   expression_holder<tensor_to_scalar_expression<value_type>>; using
//   result_expr_type = expression_holder<tensor_expression<value_type>>;

//   /**
//    * @brief Construct a differentiation visitor for derivatives w.r.t. @p
//    arg.
...
//   /**
//    * @brief Differentiate a tensor-to-scalar expression w.r.t. the stored
//    tensor
//    * argument.
...
//    *
//    * @details
//    * If the expression does not depend on \f$ \mathbf{X} \f$ (or
//    differentiation
...
//       return; // zero
//     }
//     m_result = inner_product(I, sequence{1, 2}, std::move(dA), sequence{1,
//     2});
//   }
...
//     m_result = two * inner_product(visitable.expr(), idx, std::move(dA),
//     idx);
//   }
...
//    *
//    * @note If either derivative is not valid (no dependency), it is treated
//    as
//    * zero.
//    */
...
//    * @details
//    * Let \f$g(\mathbf{X})\f$ and \f$h(\mathbf{X})\f$ be scalar-valued
//    functions
//    * and define \f[ f(\mathbf{X}) = g(\mathbf{X})^{h(\mathbf{X})}. \f] For
//    the
//    * differentiable branch (typically assuming \f$g(\mathbf{X}) > 0\f$ in the
//    * real case), rewrite \f$f\f$ via the exponential: \f[ f(\mathbf{X}) =
//    * \exp\!\big(h(\mathbf{X}) \ln(g(\mathbf{X}))\big). \f] Then the chain
//    rule
//    * gives: \f[ \frac{\partial f}{\partial \mathbf{X}} =
//    * g(\mathbf{X})^{h(\mathbf{X})} \left( \frac{\partial h}{\partial
//    \mathbf{X}}
//    * \,\ln(g(\mathbf{X}))
...
//                std::pow(visitable.expr_lhs(), visitable.expr_rhs() - one) *
//                dg;
//   }

//   /**
//    * @brief Scalar multiplication: \f$ f = c\cdot g(\mathbf{X}) \f$.
//    * @details \f$ \frac{\partial f}{\partial \mathbf{X}} = c\cdot
//    \frac{\partial
...
//    */
//   void
//   operator()([[maybe_unused]] scalar_with_tensor_to_scalar_div<ValueType>
//   const
//                  &visitable) {
//     auto dg = diff(visitable.expr_rhs(), m_arg);
//     if (!dg.is_valid()) {
...
//    * \f$\mathcal{I}_a\f$ and \f$\mathcal{I}_b\f$ denote the index lists used
//    for
//    * contraction.
//    *    * Using the product rule:
//    * \f[
//    * \frac{\partial f}{\partial \mathbf{X}} =
//    * \operatorname{inner}\!\Big(\frac{\partial \mathbf{a}}{\partial
//    \mathbf{X}},
...
//    * \f$\mathbf{X}\f$ then the corresponding derivative term is treated as
//    zero.
//    */
//   void
...
//     if (da.is_valid()) {
//       m_result =
//           inner_product(da, sequence_lhs, visitable.expr_rhs(),
//           sequence_rhs);
//     }
//     if (db.is_valid()) {
//       m_result =
//           std::move(m_result) +
//           inner_product(visitable.expr_lhs(), sequence_lhs, db,
//           sequence_rhs);
//     }
//   }
...
//    */
//   void
//   operator()([[maybe_unused]] tensor_to_scalar_scalar_wrapper<ValueType>
//   const
//                  &visitable) {
//     // set zero in apply function
//   }
