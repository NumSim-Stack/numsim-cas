...
#include <numsim_cas/scalar/scalar_node_list.h>
...
NUMSIM_CAS_SCALAR_NODE_LIST(NUMSIM_CAS_FWD_FIRST, NUMSIM_CAS_FWD_NEXT)
...
using scalar_node_types = type_list<NUMSIM_CAS_SCALAR_NODE_LIST(
    NUMSIM_CAS_TYPELIST_FIRST, NUMSIM_CAS_TYPELIST_NEXT)>;
#undef NUMSIM_CAS_TYPELIST_FIRST
#undef NUMSIM_CAS_TYPELIST_NEXT

template <typename T> struct get_index<T, scalar_expression> {
  static constexpr std::uint16_t index = index_of_v<T, scalar_node_types>;
};
...
using scalar_visitor_return_t =
    visitor_return<ReturnType, NUMSIM_CAS_SCALAR_NODE_LIST(
                                   NUMSIM_CAS_VIS_FIRST, NUMSIM_CAS_VIS_NEXT)>;
#undef NUMSIM_CAS_VIS_FIRST
#undef NUMSIM_CAS_VIS_NEXT

using scalar_visitor_return_expr_t =
    scalar_visitor_return_t<expression_holder<scalar_expression>>;

#define NUMSIM_CAS_VIS_FIRST(T) T
#define NUMSIM_CAS_VIS_NEXT(T) , T
using scalar_visitor_t = visitor<NUMSIM_CAS_SCALAR_NODE_LIST(
    NUMSIM_CAS_VIS_FIRST, NUMSIM_CAS_VIS_NEXT)>;
using scalar_visitor_const_t = visitor_const<NUMSIM_CAS_SCALAR_NODE_LIST(
    NUMSIM_CAS_VIS_FIRST, NUMSIM_CAS_VIS_NEXT)>;
#undef NUMSIM_CAS_VIS_FIRST
#undef NUMSIM_CAS_VIS_NEXT
...
using scalar_visitable_t =
    visitable<scalar_expression, NUMSIM_CAS_SCALAR_NODE_LIST(
...
template <typename T>
using scalar_node_base_t =
    visitable_impl<scalar_expression, T,
                   NUMSIM_CAS_SCALAR_NODE_LIST(NUMSIM_CAS_NB_FIRST,
                                               NUMSIM_CAS_NB_NEXT)>;
...
#endif // SCALAR_VISITOR_TYPEDEF_H
