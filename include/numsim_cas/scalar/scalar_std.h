...
template <scalar_expr_holder E> [[nodiscard]] auto tanh(E const &e) {
  ...
  // tanh(x) = 2 / (1 + exp(-2x)) - 1: a single exp term, so diff() never
  // inserts a shared exp child twice into one add, and the derivative is one
  // product that saturates for large x instead of cancelling two big terms.
  expression_holder<scalar_expression> two =
      make_expression<scalar_constant>(scalar_number{2});
  expression_holder<scalar_expression> minus_two =
      make_expression<scalar_constant>(scalar_number{-2});
  auto den = get_scalar_one() + exp(std::move(minus_two) * e);
  return std::move(two) / std::move(den) - get_scalar_one();
}
