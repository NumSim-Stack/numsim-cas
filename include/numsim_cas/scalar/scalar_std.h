...
#include <numsim_cas/scalar/scalar_io.h> // for operator<<
...
inline std::string to_string(expression_holder<scalar_expression> const &expr) {
...
auto pow(L &&expr_lhs, R expr_rhs) {
  assert(expr_lhs.is_valid());
  if (expr_rhs == static_cast<R>(1))
    return std::forward<L>(expr_lhs);
  if (expr_rhs == static_cast<R>(0))
    return get_scalar_one();
  auto constant{make_expression<scalar_constant>(expr_rhs)};
  return binary_scalar_pow_simplify(std::forward<L>(expr_lhs),
                                    std::move(constant));
}
