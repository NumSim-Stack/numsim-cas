...
[[nodiscard]] auto make_expression(Args &&...args) {
  ...
  if constexpr (variant_traits::value) {
    return expression_holder<ExprType>(
        std::make_shared<variant>(std::in_place_index<variant_traits::index>,
...
auto make_scalar_variable(Args &&...args) {
  return std::make_tuple(
      make_expression<scalar<ValueType>>(std::forward<Args>(args))...);
}

template <typename ValueType, typename... Args>
auto make_scalar_constant(Args &&...args) {
  return std::make_tuple(
      make_expression<scalar_constant<ValueType>>(std::forward<Args>(args))...);
}
