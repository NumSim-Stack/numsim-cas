...
public:
  ...
  [[nodiscard]] virtual bool like_term_of(expression const &rhs) const {
    return *this == rhs;
  }

protected:
  // each concrete node compares against same dynamic type here
  virtual bool equals_same_type(expression const &rhs) const = 0;
  virtual bool less_than_same_type(expression const &rhs) const = 0;
