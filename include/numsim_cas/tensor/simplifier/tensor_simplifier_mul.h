...
//   constexpr inline expr_type operator()(kronecker_delta const &) {
//     return std::forward<ExprLHS>(m_lhs);
//   }

//   //  template <typename _Expr, typename _ValueType>
//   //  constexpr auto get_coefficient(_Expr const &expr, _ValueType const
//   &value)
//   //  {
...
//     auto mul_new{make_expression<tensor_mul>(m_lhs.get().dim(),
//                                                          m_rhs.get().rank())};
...
//   auto operator()([[maybe_unused]] tensor const &rhs) {
//     if (lhs.expr_lhs().get().hash_value() == rhs.hash_value()) {
//       const auto rhs_expr{lhs.expr_rhs() + get_scalar_one<int>()};
//       return make_expression<tensor_pow>(lhs.expr_lhs(),
//                                                      std::move(rhs_expr));
//     }
//     return get_default();
//   }

//   auto operator()([[maybe_unused]] tensor_pow const &rhs) {
...
//   constexpr inline expr_type operator()(tensor_pow const &rhs) {
...
//   auto operator()([[maybe_unused]] tensor_mul const &rhs) {
//     auto expr_mul{make_expression<tensor_mul>(lhs)};
//     auto &mul{expr_mul.template get<tensor_mul>()};
...
//   constexpr inline expr_type operator()(tensor_pow const &) {
//     const auto &expr{*m_rhs};
//     return visit(tensor_pow_mul<ExprLHS,
//     ExprRHS>(std::forward<ExprLHS>(m_lhs),
//                                                   std::forward<ExprRHS>(m_rhs)),
//                  expr);
//   }
...
//   constexpr inline expr_type operator()(tensor_mul const &) {
