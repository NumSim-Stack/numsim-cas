...
constexpr inline auto binary_mul_tensor_with_scalar_simplify(ExprTypeLHS &&lhs, ExprTypeRHS &&rhs);
...
//template <typename ExprLHS, typename ExprRHS>
//class tensor_scalar_mul_simplifier final : public mul_default<ExprLHS, ExprRHS>{
//public:
//  using value_type = typename std::remove_reference_t<
//      std::remove_const_t<ExprLHS>>::value_type;
//  using expr_lhs = typename std::remove_reference_t<
...
//  tensor_scalar_mul_simplifier(expr_lhs lhs, expr_rhs rhs):base(lhs,rhs),lhs{base::m_lhs.template get<tensor_scalar_mul<value_type>>()}
//  {}

//  constexpr inline expr_type operator()(scalar_expression<value_type> const& ) {
//    std::cout<<"constexpr inline expr_type operator()(scalar_expression<value_type> const& ) {"<<std::endl;
//    //auto scalar_expr = lhs.expr_lhs() * m_rhs;
//    return make_expression<tensor_scalar_mul<value_type>>(lhs.expr_lhs() * m_rhs, lhs.expr_rhs());
//    //return expr_type{};
//  }
...
public:
  ...
  scalar_tensor_mul(expr_lhs lhs, expr_rhs rhs):base(lhs,rhs),lhs{base::m_lhs.template get<scalar_expression<value_type>>()}
  {}

  constexpr inline expr_type operator()(tensor_scalar_mul<value_type> const& rhs) {
    return make_expression<tensor_scalar_mul<value_type>>(rhs.expr_lhs() * m_lhs, rhs.expr_rhs());
  }
...
struct mul_base
{
  ...
  //    return visit(tensor_scalar_mul_simplifier<expr_lhs, expr_rhs>(m_lhs,m_rhs), *m_rhs);
  //  }

  //  constexpr inline expr_type operator()(scalar_expression<value_type> const&){
  //    return visit(scalar_tensor_mul_simplifier<expr_lhs, expr_rhs>(m_lhs,m_rhs), *m_rhs);
  //  }
