...
public:
  ...
  //    auto add_new{make_expression<tensor_add>()};
...
//  template<typename Expr>
//  constexpr inline expr_type operator()(Expr const&){
//    return make_expression<tensor_scalar_mul>(m_rhs, m_lhs);
//  }
// private:
//  using base::m_lhs;
//  using base::m_rhs;
...
public:
  ...
  scalar_tensor_mul(ExprLHS &&lhs, ExprRHS &&rhs)
      : base(std::forward<ExprLHS>(lhs), std::forward<ExprRHS>(rhs)),
        lhs{base::m_lhs.template get<scalar_expression>()} {}

  constexpr inline expr_type operator()(tensor_scalar_mul const &rhs) {
...
template <typename ExprLHS, typename ExprRHS> struct mul_base {
  ...
  //  constexpr inline expr_type operator()(tensor_scalar_mul
  //  const&){
  //    return visit(tensor_scalar_mul_simplifier<expr_lhs,
  //    expr_rhs>(m_lhs,m_rhs), *m_rhs);
  //  }

  //  constexpr inline expr_type operator()(scalar_expression
  //  const&){
  ...
  template <typename Expr,
            std::enable_if_t<!std::is_base_of_v<scalar_expression, Expr>,
                             bool> = true>
  constexpr inline expr_type operator()(Expr const &) {
    static_assert(true, "not supported");
  }
