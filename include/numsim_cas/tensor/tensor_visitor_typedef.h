#ifndef TENSOR_VISITOR_TYPEDEF_H
#define TENSOR_VISITOR_TYPEDEF_H

#include <cstdint>
#include <numsim_cas/core/visitor_base.h>
#include <numsim_cas/tensor/tensor_node_list.h>
...
NUMSIM_CAS_TENSOR_NODE_LIST(NUMSIM_CAS_FWD_FIRST, NUMSIM_CAS_FWD_NEXT)
...
using tensor_node_types = type_list<NUMSIM_CAS_TENSOR_NODE_LIST(
    NUMSIM_CAS_TYPELIST_FIRST, NUMSIM_CAS_TYPELIST_NEXT)>;
#undef NUMSIM_CAS_TYPELIST_FIRST
#undef NUMSIM_CAS_TYPELIST_NEXT

template <typename T> struct get_index<T, tensor_expression> {
  static constexpr std::uint16_t index = index_of_v<T, tensor_node_types>;
};
...
using tensor_visitor_return_t =
    visitor_return<ReturnType, NUMSIM_CAS_TENSOR_NODE_LIST(
                                   NUMSIM_CAS_VIS_FIRST, NUMSIM_CAS_VIS_NEXT)>;
#undef NUMSIM_CAS_VIS_FIRST
#undef NUMSIM_CAS_VIS_NEXT

using tensor_visitor_return_expr_t =
    tensor_visitor_return_t<expression_holder<tensor_expression>>;

#define NUMSIM_CAS_VIS_FIRST(T) T
#define NUMSIM_CAS_VIS_NEXT(T) , T
using tensor_visitor_t = visitor<NUMSIM_CAS_TENSOR_NODE_LIST(
    NUMSIM_CAS_VIS_FIRST, NUMSIM_CAS_VIS_NEXT)>;
using tensor_visitor_const_t = visitor_const<NUMSIM_CAS_TENSOR_NODE_LIST(
    NUMSIM_CAS_VIS_FIRST, NUMSIM_CAS_VIS_NEXT)>;
#undef NUMSIM_CAS_VIS_FIRST
#undef NUMSIM_CAS_VIS_NEXT
...
using tensor_visitable_t =
    visitable<tensor_expression, NUMSIM_CAS_TENSOR_NODE_LIST(
                                     NUMSIM_CAS_VT_FIRST, NUMSIM_CAS_VT_NEXT)>;
...
using tensor_node_base_t =
    visitable_impl<tensor_expression, T,
                   NUMSIM_CAS_TENSOR_NODE_LIST(NUMSIM_CAS_NB_FIRST,
                                               NUMSIM_CAS_NB_NEXT)>;
...
#endif // TENSOR_VISITOR_TYPEDEF_H
