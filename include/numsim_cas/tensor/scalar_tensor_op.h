...
struct tensor_scalar_mul{
  ...
  static constexpr inline auto apply(Scalar const& scalar, Tensor const& tensor){
...
class scalar_tensor_op final : public tensor_data_eval_up_unary<scalar_tensor_op<OP, ValueType>, ValueType>
{
public:
  scalar_tensor_op(tensor_data_base<ValueType> &lhs, ValueType const &scalar, tensor_data_base<ValueType> const &rhs)
  ...
  inline void evaluate_imp() noexcept {
    ...
    using Tensor = tensor_data<T, Dim, Rank>;
    static_cast<Tensor &>(m_lhs).data() += OP::apply(m_scalar, static_cast<const Tensor &>(m_rhs).data());
  }
...
using scalar_tensor_mul_op = scalar_tensor_op<detail::tensor_scalar_mul, double>;
using scalar_tensor_div_op = scalar_tensor_op<detail::tensor_scalar_div, double>;
