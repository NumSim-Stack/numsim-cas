...
public:
  ...
  [[nodiscard]] virtual const ValueType *
  raw_data() const noexcept = 0;
  [[nodiscard]] virtual ValueType *raw_data() noexcept = 0;
  virtual void print(std::ostream& out) noexcept = 0;
  template<typename T>
  inline friend std::ostream& operator <<(std::ostream& out, tensor_data_base<T> &);
};

template<typename ValueType>
inline std::ostream& operator <<(std::ostream& out, tensor_data_base<ValueType> & tensor){
  tensor.print(out);
  return out;
}


template <typename ValueType, std::size_t Dim, std::size_t Rank> class tensor_data final : public tensor_data_base<ValueType> {
public:
  using data_type = tmech::tensor<ValueType, Dim, Rank>;
  tensor_data() : m_data() {}

  explicit tensor_data(data_type const &data) : tensor_data_base<ValueType>(), m_data(data) {}
  ...
  raw_data() const noexcept final override {
