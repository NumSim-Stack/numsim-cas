#ifndef TENSOR_SUBSTITUTION_H
#define TENSOR_SUBSTITUTION_H
...
// Forward declare tensor tag_invoke overloads.
template <class TargetBase>
expression_holder<tensor_expression>
tag_invoke(detail::substitute_fn, std::type_identity<tensor_expression>,
...
class tensor_substitution final : public tensor_rebuild_visitor {
public:
  using target_holder_t = expression_holder<TargetBase>;

  tensor_substitution(target_holder_t const &old_val,
                      target_holder_t const &new_val)
      : m_old(old_val), m_new(new_val) {}

  tensor_holder_t apply(tensor_holder_t const &expr) noexcept override {
  ...
  t2s_holder_t apply_t2s(t2s_holder_t const &expr) noexcept override {
    return substitute(expr, m_old, m_new);
  }
...
tag_invoke(detail::substitute_fn, std::type_identity<tensor_expression>,
  ...
  tensor_substitution<TargetBase> visitor(old_val, new_val);
  return visitor.apply(expr);
}

} // namespace numsim::cas

#endif // TENSOR_SUBSTITUTION_H
