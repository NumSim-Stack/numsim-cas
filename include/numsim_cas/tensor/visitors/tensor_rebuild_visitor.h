...
public:
  ...
  virtual tensor_holder_t apply(tensor_holder_t const &expr) noexcept {
    if (expr.is_valid()) {
      m_current = expr;
  ...
  apply_scalar(scalar_holder_t const &expr) noexcept {
    return expr;
  }

  virtual t2s_holder_t apply_t2s(t2s_holder_t const &expr) noexcept {
    return expr;
  }

  // Leaf nodes: return as-is
  void operator()(tensor const &) noexcept override { m_result = m_current; }
  void operator()(tensor_zero const &) noexcept override {
    m_result = m_current;
  }
  void operator()(kronecker_delta const &) noexcept override {
    m_result = m_current;
  }
  void operator()(identity_tensor const &) noexcept override {
    m_result = m_current;
  }
  void operator()(tensor_projector const &) noexcept override {
    m_result = m_current;
  }
  ...
  void operator()(basis_change_imp const &v) noexcept override {
    m_result =
        make_expression<basis_change_imp>(apply(v.expr()), v.indices());
  }

  // Binary tensor x tensor -> tensor
  void operator()(inner_product_wrapper const &v) noexcept override {
    m_result = make_expression<inner_product_wrapper>(
        apply(v.expr_lhs()), v.indices_lhs(), apply(v.expr_rhs()),
        v.indices_rhs());
  }

  void operator()(outer_product_wrapper const &v) noexcept override {
  ...
  void operator()(tensor_mul const &v) noexcept override {
    tensor_holder_t result;
    if (v.coeff().is_valid())
      result *= apply(v.coeff());
  ...
  void operator()(simple_outer_product const &v) noexcept override {
