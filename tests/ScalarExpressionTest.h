...
TYPED_TEST(ScalarFixture, DIFF_LinearityAndBasics) {
  ...
  EXPECT_PRINT(numsim::cas::diff(a * x + b * y + c, x),
               "a");                              // a,b,c independent of x
  EXPECT_PRINT(numsim::cas::diff(x * y, x), "y"); // treat y as const
  EXPECT_PRINT(numsim::cas::diff((x + y) * x, x),
               "2*x+y"); // product rule + simplification
  EXPECT_PRINT(numsim::cas::diff(x / y, x),
               "pow(y,-1)"); // constant denominator
}
...
TYPED_TEST(ScalarFixture, DIFF_ChainRule) {
  auto &x = this->x;
  EXPECT_PRINT(numsim::cas::diff(pow(x + this->_1, this->_3), x),
               "3*pow(1+x,2)");
}
...
// (keep these expectations until you implement cancellation; pow/x does
// reduce.)
//
...
//   EXPECT_EQ(std::to_string(numsim::cas::diff(x * x * x * x, x)),
//   "4*pow(x,3)");
...
//   product ordering EXPECT_EQ(std::to_string(y + x + z + y + x), "2*x+2*y+z");
//   // sum collection + ordering EXPECT_EQ(std::to_string(this->_2 + x),
//   "2+x");                          // constant first
...
//   auto &x = this->x, &y = this->y;
//   EXPECT_EQ(std::to_string(-(x + y)), "-(x+y)");      // wrap sums in parens
//   under negation EXPECT_EQ(std::to_string(-(-x)), "x");              //
//   double negation EXPECT_EQ(std::to_string(x + (-x)), "0");           //
//   additive inverse
// }
...
//   EXPECT_EQ(std::to_string(pow(x, y) * x), "pow(x,1+y)");   // symbolic
//   exponent bump
// }
...
//   auto &x = this->x, &y = this->y, &a = this->a, &b = this->b, &c = this->c;
//   EXPECT_EQ(std::to_string(numsim::cas::diff(this->_2 * x + this->_3, x)),
//   "2"); EXPECT_EQ(std::to_string(numsim::cas::diff(a * x + b * y + c, x)),
//   "a"); // a,b,c independent of x
//   EXPECT_EQ(std::to_string(numsim::cas::diff(x * y, x)), "y");             //
//   product rule w/ const factor EXPECT_EQ(std::to_string(numsim::cas::diff((x
//   + y) * x, x)), "2*x+y");   // product rule + simplification
//   EXPECT_EQ(std::to_string(numsim::cas::diff(x / y, x)), "pow(y,-1)"); //
//   quotient rule w/ const denom
// }
...
//   EXPECT_EQ(std::to_string(numsim::cas::diff(pow(x + this->_1, this->_3),
//   x)), "3*pow(1+x,2)");
// }

// TYPED_TEST(ScalarExpressionTest, ScalarMixedCollection) {
//   auto &x = this->x, &y = this->y, &z = this->z;
//   EXPECT_EQ(std::to_string((x + y + z) + (y + x)), "2*x+2*y+z"); // cross-sum
//   collection EXPECT_EQ(std::to_string(this->_2 * (this->_3 + x)), "2*(3+x)");
//   // no forced distribution; constant first inside
// }
...
//   auto &x = this->x, &y = this->y;
//   auto &_1 = this->_1, &_2 = this->_2;
//   EXPECT_EQ(std::to_string((x + y) + _2), "2+x+y");   // exercises lvalue
//   branch (bug #1) EXPECT_EQ(std::to_string(_2 + (x + y)), "2+x+y");   //
//   symmetric path EXPECT_EQ(std::to_string((x + _1) + _2), "3+x");
