...
TEST(TensorAlgebraDetPropagation, DetScalarMulPdLosesPositivityViaComposition) {
  ...
  // (recursive call hits the propagation), but the outer t2s mul
  // strips it. Documenting the gap.
  auto C = std::get<0>(make_tensor_variable(std::tuple{"C", 3, 2}));
  auto alpha = make_scalar_variable("alpha");
  assume_positive_definite(C);
  auto d = det(std::get<0>(alpha) * C);
  // CURRENT: outer composition (pow * det) lacks positive. If/when
  // #260 lands, flip this expectation.
...
// ─── Cross-PR composition tests (integration of α-2a/b/c/d + #263) ──
// These tests exercise CHAINS that touch multiple α-2 PRs. Each one
...
TEST(TensorAlgebraComposition, EndToEndChainDetInvTimesOrthogonalIsPositive) {
  ...
  //   inv(R)              → trans(R)            (α-2a inv fold)
  //   trans(R)            annotated orthogonal  (α-2a trans propagation)
  //   trans(R) * R        → identity_tensor     (α-2c mul fold)
  //   det(identity)       → tensor_to_scalar_one (existing fold)
  //   tensor_to_scalar_one annotated positive    (#263 constants)
...
TEST(TensorAlgebraComposition, InvRank1RankGateBeatsOrthogonalFold) {
  // Rank-1 edge case: a rank-1 tensor with the orthogonal annotation
  // (user error — orthogonality is rank-2 algebra). The inv() factory's
  // rank gate (rank != 2 && rank != 4) fires BEFORE the α-2a
  // orthogonal fold, throwing the "only rank-2 and rank-4" error.
  // Locks in the ordering: rank gate is a safety check that takes
  // precedence over the algebra fold.
  auto V = make_expression<tensor>("V", std::size_t{3}, std::size_t{1});
  assume_orthogonal(V);
  try {
    [[maybe_unused]] auto r = inv(V);
    FAIL() << "Expected inv(rank-1) to throw 'only rank-2 and rank-4'";
  } catch (invalid_expression_error const &e) {
    EXPECT_NE(std::string(e.what()).find("rank"), std::string::npos)
        << "error should mention rank; got: " << e.what();
  }
}
