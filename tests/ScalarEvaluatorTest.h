...
template <typename T> struct ScalarEvalFixture : ::testing::Test {
  ...
  inline static void expect_eq_or_near(value_type actual, value_type expected) {
...
TYPED_TEST(ScalarEvalFixture, EVAL_Fundamentals) {
  ...
  this->expect_eq_or_near(ev.apply(_1 + _2 + _3), static_cast<value_type>(6));
  this->expect_eq_or_near(ev.apply(_2 * _3), static_cast<value_type>(6));
...
TYPED_TEST(ScalarEvalFixture, EVAL_CoversAllNodeTypes) {
  ...
  if constexpr (std::is_floating_point_v<value_type>) {
    ...
    this->expect_eq_or_near(ev.apply(asin(zero)), static_cast<value_type>(0));
    // acos(0) = pi/2 -> truncates to 1 for int (implementation uses std::acos
    // -> double -> int)
    this->expect_eq_or_near(ev.apply(acos(zero)),
                            static_cast<value_type>(std::acos(0.0)));
    this->expect_eq_or_near(ev.apply(atan(zero)), static_cast<value_type>(0));
    ...
    // sqrt(4) via sqrt(1+3)
    auto four = _1 + _3;
    this->expect_eq_or_near(ev.apply(sqrt(four)), static_cast<value_type>(2));

    // pow(2,3) = 8
    this->expect_eq_or_near(ev.apply(pow(_2, _3)), static_cast<value_type>(8));
...
//
// EVAL_EXCEPTIONS — division by zero, sqrt domain, log domain (floating only)
//
