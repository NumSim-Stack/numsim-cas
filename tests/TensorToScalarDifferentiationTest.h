...
//   auto expected = (detY * inv(trans(Y))) * log(trY) + detY * (I<T>(dim) /
//   trY); EXPECT_SAME_PRINT(d, expected);
// }
...
// Combo_NonDependencyInsideExpression) {
//   tensor_to_scalar_differentiation<T> dY(Y);
...
// // Regression-style combo: ensures apply() does not leak results between
// calls. TEST_F(TensorToScalarDifferentiationTest,
