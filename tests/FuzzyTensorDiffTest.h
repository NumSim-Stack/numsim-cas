...
// over the 8-element D_4 group generated by σ_1 (swap i,j), σ_2 (swap
// k,l), and σ_M (pair-swap of (i,j) ↔ (k,l) — i.e. exchange the two
// index PAIRS, not a transposition within a pair). Matches the
// rank-4 MinorMajor inv-diff branch (#250).
...
private:
  ...
  void create_variable_pool() {
    ...
    // the math says shouldn't happen. Cause confirmed by inspection:
    // the leaf rule at tensor_differentiation.h:69-98 has a rank-2
    // annotation branch (returns P_sym/P_skew/P_vol/P_devi) but NO
    // analogous rank-4 branch — it returns the unconstrained rank-8
    // identity for diff(M_min, M_min) instead of the projected
    ...
    //
    // Compile-test: actually instantiate the projection helpers (the
    // inline make_*_projection functions would otherwise get elided
    // since nothing in the TU ODR-uses them). The if (false) prevents
    // runtime execution; the compiler still has to instantiate the
    // call.
    if (false) {
      add_var_minor4("__compile_test_minor");
      add_var_minor_major4("__compile_test_mm");
    }
  }
