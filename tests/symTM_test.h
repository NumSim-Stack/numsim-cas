...
// convert(numsim::cas::tensor_data_base<ValueType> &tensor) {
//   return static_cast<numsim::cas::tensor_data<ValueType, Dim, Rank>
//   &>(tensor);
// }
...
//                     numsim::cas::get_tensor<ValueType, Dim, Rank>(result),
//                     a + b + c, 5e-7));
// }
...
//        numsim::cas::tuple("a", Dim, RankLHS),                                 \
//        numsim::cas::tuple("b", Dim, RankRHS))};                               \
...
//    asym.get<numsim::cas::tensor<ValueType>>() = a;                            \
//    bsym.get<numsim::cas::tensor<ValueType>>() = b;                            \
//    const auto result{numsim::cas::evaluate(                                   \
//        numsim::cas::inner_product(asym, Seq{3, 4}, bsym, Seq{1, 2}))};        \
//    auto result_t{                                                             \
//        tmech::inner_product<tmech::sequence<3, 4>, tmech::sequence<1, 2>>(    \
//            a, b)};                                                            \
//    EXPECT_EQ(                                                                 \
//        true,                                                                  \
//        tmech::almost_equal(                                                   \
//            numsim::cas::get_tensor<ValueType, Dim, RankLHS + RankRHS - 4>(    \
//                result),                                                       \
//            result_t, 5e-7));                                                  \
//  }
...
//    bsym.get<numsim::cas::tensor<ValueType>>() = b;                            \
//    const auto result{numsim::cas::evaluate(                                   \
//        numsim::cas::inner_product(asym, Seq{1, 2}, bsym, Seq{3, 4}))};        \
//    auto result_t{                                                             \
//        tmech::inner_product<tmech::sequence<1, 2>, tmech::sequence<3, 4>>(    \
//            a, b)};                                                            \
//    EXPECT_EQ(                                                                 \
//        true,                                                                  \
//        tmech::almost_equal(                                                   \
...
//    asym.get<numsim::cas::tensor<ValueType>>() = a;                            \
//    const auto result{numsim::cas::evaluate(                                   \
//        numsim::cas::basis_change(asym, Seq{__VA_ARGS__}))};                   \
...
//    bsym.get<numsim::cas::tensor<ValueType>>() = b;                            \
//    const auto result{numsim::cas::evaluate(                                   \
//        numsim::cas::basis_change(asym + bsym, Seq{__VA_ARGS__}))};            \
...
//    asym.get<numsim::cas::tensor<ValueType>>() = a;                            \
//    bsym.get<numsim::cas::tensor<ValueType>>() = b;                            \
//    const auto result{numsim::cas::evaluate(numsim::cas::inner_product(        \
//        asym + asym, Seq{3, 4}, bsym + bsym, Seq{1, 2}))};                     \
//    auto result_t{                                                             \
//        tmech::inner_product<tmech::sequence<3, 4>, tmech::sequence<1, 2>>(    \
//            a + a, b + b)};                                                    \
//    EXPECT_EQ(                                                                 \
...
//    bsym.get<numsim::cas::tensor<ValueType>>() = b;                            \
//    const auto result{numsim::cas::evaluate(numsim::cas::inner_product(        \
//        asym + asym, Seq{1, 2}, bsym + bsym, Seq{3, 4}))};                     \
//    auto result_t{                                                             \
//        tmech::inner_product<tmech::sequence<1, 2>, tmech::sequence<3, 4>>(    \
//            a + a, b + b)};                                                    \
//    EXPECT_EQ(                                                                 \
